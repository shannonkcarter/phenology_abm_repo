...
setwd("/Users/shannoncarter/Documents/phenology-abm")
rm(list = ls(all = T))
...
gr <- ind_time %>% 
  filter(id == 30) %>% 
  filter(sync %in% c(3, 15, 27)) %>%
  mutate(synchrony = case_when(sync == 3 ~ "high synchrony",
                               sync == 15 ~ "medium synchrony",
                               sync == 27 ~ "low synchrony"),
         synchrony = factor(synchrony, levels = c("low synchrony", "medium synchrony", "high synchrony"), ordered = T)) %>% 
  ggplot(aes(x = time, y = size, group = run_num)) + 
  geom_line() +
